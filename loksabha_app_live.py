...
def get_total_seats(_engine):
    """Q1"""
    df = pd.read_sql("SELECT COUNT(*) AS total_seats FROM loksabha.constituencywise_results", _engine)
...
def get_seats_by_state(_engine):
    ...
    query = """
        ...
        FROM loksabha.constituencywise_results t1
        JOIN loksabha.statewise_results t2 ON t1.`Parliament Constituency` = t2.`Parliament Constituency`
        JOIN loksabha.stats t3 ON t2.`State ID` = t3.`State ID`
        GROUP BY t3.State
...
def get_seats_by_party(_engine):
    ...
    query = """
        ...
        FROM loksabha.partywise_results t1
        JOIN loksabha.constituencywise_results t2 ON t1.`Party ID` = t2.`Party ID`
        GROUP BY t1.Party
...
def get_india_alliance_total(_engine):
    ...
    query = """
        ...
        FROM loksabha.partywise_results
    """
...
def get_constituency_vote_split(_engine, constituency_name):
    ...
    query = text("""
        ...
        FROM loksabha.constituencywise_details t1
        JOIN loksabha.constituencywise_results t2 ON t1.`Constituency ID` = t2.`Constituency ID`
...
def get_party_seats_in_state(_engine, state_name):
    ...
    query = text("""
        ...
        FROM loksabha.constituencywise_results t1
        JOIN loksabha.partywise_results t2 ON t1.`Party ID` = t2.`Party ID`
        JOIN loksabha.statewise_results t3 ON t1.`Parliament Constituency` = t3.`Parliament Constituency`
        JOIN loksabha.stats t4 ON t3.`State ID` = t4.`State ID`
...
def get_state_list(_engine):
    return pd.read_sql("SELECT DISTINCT State FROM loksabha.stats ORDER BY State", _engine)["State"].tolist()


@st.cache_data(ttl=600)
def get_constituency_list(_engine):
    return pd.read_sql(
        "SELECT DISTINCT `Constituency Name` FROM loksabha.constituencywise_results ORDER BY `Constituency Name`",
        _engine,
...
def get_winner_runnerup(_engine):
    ...
    query = """
        WITH RankedCandidates AS (
            ...
            FROM loksabha.constituencywise_results t1
            JOIN loksabha.statewise_results t2 ON t1.`Parliament Constituency` = t2.`Parliament Constituency`
            JOIN loksabha.constituencywise_details t3 ON t1.`Constituency ID` = t3.`Constituency ID`
        )
